...
const Todo = ({ todo }: TodoProps) => {
  ...
  const handleEdit = async () => {
    ...
    if (isEditing) {
      ...
      if (response.ok) {
        ...
        const updatedTodo = todos.map((todo: TodoType) => todo.id === editedTodo.id ? editedTodo :todo);
        mutate(updatedTodo);
      }
    }
  };
  ...
  const handleDelete = async (id:number) => {
    ...
    if (response.ok) {
      const updatedTodo = todos.filter((todo: TodoType) => todo.id !== id);
      mutate(updatedTodo);
    };
  }; 
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <label className="ml-3 block text-gray-900">
              {isEditing ? (
                <input type="text" className='border rouned py-1 px-2' value={editedTitle} onChange={(e) => setEditedTitle(e.target.value)}/>
              ) : (
